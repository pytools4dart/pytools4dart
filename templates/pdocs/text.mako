...
```
    ...
    ## parse Numpy-style docstring: set subsections as h6
    ## convert section title to md level 6 so it can be filtered with toc_depth
    ## <%
    ##     import re
    ##     docstring = func.docstring
    ##     lines = []
    ##     for i, line in enumerate(docstring.splitlines()):
    ##         if (re.match('^-+$', line) is not None) and i>0 :
    ##            lines[i-1] = "###### "+ lines[i-1]
    ##         lines.append(re.sub("^-+$", "", line))
    ##     docstring = '\n'.join(lines)
    ## %>

## Docstring is displayed in collapsed box
## To remove collapsible box, comment the following line (??? example ...) and
## ??? example "Details"
